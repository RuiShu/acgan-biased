...
def recode(x,
    ...
    with tf.variable_scope(scope, 'recode', reuse=reuse):
        with arg_scope([dense], activation=activation, bn=bn, phase=phase):

            x = dense(x, num_hidden)
            x = dense(x, num_hidden)
            m = dense(x, num_outputs, activation=None)
            v = dense(x, num_outputs, activation=tf.nn.softplus) + 1e-5

        return (m, v)

@add_arg_scope
def infer(likelihood, prior, scope=None, reuse=None):

    with tf.variable_scope(scope, 'infer', reuse=reuse):
        if prior is None:
            ...
            posterior = gaussian_update(*args)
